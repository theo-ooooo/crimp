...
SELECT m.id, 1, 3, 'image/webp', m.byte_size, m.width, m.height, m.webp_path, TRUE
FROM media_assets m
...
SELECT m.id, 1, 3, m.mime, m.byte_size, m.width, m.height, m.duration_ms, m.original_path, TRUE
FROM media_assets m
