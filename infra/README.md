...
> 포트 13306 은 Homebrew 등 로컬 다른 MySQL 인스턴스(기본 3306)와 충돌 방지를 위함.
...
- 패키지 `@benborla29/mcp-server-mysql` 의 env 이름 규약: `MYSQL_PASS`, `MYSQL_DB` (축약형)
- `--scope project` 플래그로 `.mcp.json`에 저장하면 팀 전체가 공유 가능 (단, 커밋 전에 민감정보 환경 변수 참조로 돌려야 함).
- 등록 후 `/exit` → `claude --continue` 로 세션 재시작해야 도구가 로드됨.
